...
let selectedClass = "kelas_x"; // Default Kelas X
let selectedSubject = "matematika"; // Default Matematika

// CATALOG REGISTRY DATASET MAPEL & KELAS (PHASE 5)
const DATASET_CATALOG = {
  kelas_x: {
    name: "Kelas X",
    subjects: [
      { id: "b_indonesia", name: "Bahasa Indonesia", icon: "📚" },
      { id: "b_inggris", name: "Bahasa Inggris", icon: "🔤" },
      ...
      { id: "sosiologi", name: "Sosiologi", icon: "👥" }
    ]
  },
  kelas_xi: {
    name: "Kelas XI",
...
// Bobot probabilitas pengambilan soal berdasarkan tingkat kesulitan game yang dipilih (Easy, Standard, Hard)
...
const selectGameClass = (classId) => {
  ...
  // Re-render daftar mata pelajaran sesuai kelas
  renderSubjectsForClass(classId);
};

// Render Mata Pelajaran secara dinamis berdasarkan Kelas (Phase 5)
const renderSubjectsForClass = (classId) => {
  const subjectGrid = document.getElementById("subjectGrid");
  if (!subjectGrid) return;

  const catalog = DATASET_CATALOG[classId];
  if (!catalog || !Array.isArray(catalog.subjects)) {
    subjectGrid.innerHTML = "<p>Data mata pelajaran tidak tersedia.</p>";
    return;
  }

  // Jika mapel aktif tidak ada di kelas baru, sesuaikan ke mapel pertama
  const availableSubjIds = catalog.subjects.map((s) => s.id);
  if (!availableSubjIds.includes(selectedSubject)) {
    selectedSubject = availableSubjIds[0] || "matematika";
  }

  let html = "";
  catalog.subjects.forEach((subj) => {
    const isSelected = subj.id === selectedSubject ? "selected" : "";
    html += `
      <div class="subject-card ${isSelected}" id="subj-${subj.id}" onclick="selectGameSubject('${subj.id}')">
...
// Pemilihan Mata Pelajaran (Phase 5)
const selectGameSubject = (subjectId) => {
  selectedSubject = subjectId;
  console.log("Memilih Mata Pelajaran:", subjectId);

  document.querySelectorAll(".subject-card").forEach((btn) => btn.classList.remove("selected"));
  const activeSubjBtn = document.getElementById(`subj-${subjectId}`);
...
const setGameDifficulty = (level) => {
  ...
  if (!validLevels.includes(level)) {
    console.warn(`[Difficulty] Invalid difficulty level '${level}', fallback to 'standard'`);
    level = "standard";
  }
...
// ===== PHASE 4: QUESTION ENGINE & SCHEMA VALIDATION =====
const QuestionEngine = {
  // Mapping prefix resmi ID soal
  prefixes: {
    easy: "E-",
    standard: "S-",
    hard: "H-",
  ...
  // Helper API Ekstensibilitas (Phase 13)
  registerClass: (classId, className, subjects = []) => {
    if (!DATASET_CATALOG[classId]) {
      DATASET_CATALOG[classId] = { name: className, subjects: [] };
    }
    if (Array.isArray(subjects)) {
      subjects.forEach((s) => QuestionEngine.registerSubject(classId, s));
    }
    console.log(`[Extensibility] Kelas '${classId}' (${className}) berhasil didaftarkan.`);
  },

  registerSubject: (classId, subjectObj) => {
    if (!subjectObj || !subjectObj.id || !subjectObj.name) return;
    if (!DATASET_CATALOG[classId]) {
      DATASET_CATALOG[classId] = { name: classId, subjects: [] };
    }
    const existing = DATASET_CATALOG[classId].subjects.find((s) => s.id === subjectObj.id);
    if (!existing) {
      DATASET_CATALOG[classId].subjects.push({
        id: subjectObj.id,
        name: subjectObj.name,
        icon: subjectObj.icon || "📖"
      });
      console.log(`[Extensibility] Mapel '${subjectObj.name}' berhasil didaftarkan ke '${classId}'.`);
    }
  },

  // Validasi dan normalisasi dataset JSON (Phase 12: Hardened Validation & Error Handling)
  validateAndNormalizeDataset: (data, expectedDifficulty) => {
    if (!data || typeof data !== "object") {
      console.error(`[DataValidation] Gagal memuat soal: Format JSON tidak valid atau corrupt untuk ${expectedDifficulty}`);
      return null;
    }

    // Normalisasi property difficulty
    let diff = (data.difficulty || expectedDifficulty || "easy").toLowerCase().trim();
    if (diff === "standar") diff = "standard";
    if (!["easy", "standard", "hard", "mystery"].includes(diff)) {
      console.warn(`[DataValidation] Warning: Difficulty '${data.difficulty}' tidak sesuai baku, diset ke '${expectedDifficulty}'`);
      diff = expectedDifficulty || "easy";
    }
    ...
    // Helper pendaftar ID unik (cegah Duplicate ID)
    const getUniqueID = (rawId, prefix, idx) => {
      let finalId = rawId ? String(rawId).trim() : `${prefix}${(idx + 1).toString().padStart(2, "0")}`;
      if (seenIds.has(finalId)) {
        console.warn(`[DataValidation] Duplicate ID terdeteksi: '${finalId}'. Membuat ID unik baru.`);
        let counter = 1;
        while (seenIds.has(`${finalId}_dup${counter}`)) counter++;
        finalId = `${finalId}_dup${counter}`;
      }
      seenIds.add(finalId);
      return finalId;
    };

    // 1. Validasi Multiple Choice
    if (Array.isArray(data.multiple_choice)) {
      data.multiple_choice.forEach((q, idx) => {
        if (!q || typeof q !== "object") return;
        if (!q.question || typeof q.question !== "string" || !q.question.trim()) {
          console.warn(`[DataValidation] MCQ [${idx}] diabaikan: Pertanyaan kosong.`);
          return;
        }
        if (!Array.isArray(q.options) || q.options.length < 2) {
          console.warn(`[DataValidation] MCQ [${idx}] diabaikan: Opsi kurang dari 2.`);
          return;
        }
        let ansIndex = typeof q.answer === "number" ? q.answer : parseInt(q.answer, 10);
        if (isNaN(ansIndex) || ansIndex < 0 || ansIndex >= q.options.length) {
          console.warn(`[DataValidation] MCQ [${idx}] Answer index ${q.answer} tidak valid. Diset ke 0.`);
          ansIndex = 0;
        }
        let tLimit = parseInt(q.time_limit, 10);
        if (isNaN(tLimit) || tLimit <= 0) tLimit = 30;

        normalized.multiple_choice.push({
          id: getUniqueID(q.id, `${QuestionEngine.prefixes[diff]}MC-`, idx),
    ...
    // 2. Validasi True / False
    if (Array.isArray(data.true_false)) {
      data.true_false.forEach((q, idx) => {
        if (!q || typeof q !== "object") return;
        if (!q.question || typeof q.question !== "string" || !q.question.trim()) return;
        let ansBool = q.answer;
        if (typeof ansBool !== "boolean") {
          ansBool = String(ansBool).toLowerCase() === "true" || String(ansBool) === "1";
        }
        let tLimit = parseInt(q.time_limit, 10);
        if (isNaN(tLimit) || tLimit <= 0) tLimit = 20;

        normalized.true_false.push({
          id: getUniqueID(q.id, `${QuestionEngine.prefixes[diff]}TF-`, idx),
          question: q.question.trim(),
          answer: ansBool,
          explanation: q.explanation ? String(q.explanation).trim() : "",
          time_limit: tLimit,
    ...
    // 3. Validasi Essay & Legacy What If
    const essaySource = [
      ...(Array.isArray(data.essay) ? data.essay : []),
      ...(Array.isArray(data.what_if) ? data.what_if : [])
    ];

    essaySource.forEach((q, idx) => {
      if (!q || typeof q !== "object") return;
      if (!q.question || typeof q.question !== "string" || !q.question.trim()) return;
      if (q.answer === undefined || q.answer === null) return;
      let validInputs = ["text", "math", "set"];
      let inputType = (q.input_type || "text").toLowerCase().trim();
      if (!validInputs.includes(inputType)) inputType = "text";
      let tLimit = parseInt(q.time_limit, 10);
      if (isNaN(tLimit) || tLimit <= 0) tLimit = 45;

      normalized.essay.push({
        id: getUniqueID(q.id, `${QuestionEngine.prefixes[diff]}ES-`, idx),
        question: q.question.trim(),
    ...
    const totalQuestions = normalized.multiple_choice.length + normalized.true_false.length + normalized.essay.length;
    if (totalQuestions === 0) {
      console.warn(`[DataValidation] Dataset '${expectedDifficulty}' tidak memiliki soal valid.`);
      return null;
    }
...
// Sub-modul Modular Question Engine (Phase 7)
QuestionEngine.Loader = {
  load: async (classId, subjectId, difficultyId) => {
    return await loadQuestions(classId, subjectId, difficultyId);
  }
};
...
QuestionEngine.Manager = {
  getRandomQuestionForTile: (tileType) => {
    ...
    if (!pool) {
      console.warn(`[QuestionManager] Dataset untuk '${tileType}' tidak ditemukan. Menggunakan fallback.`);
...
QuestionEngine.Renderer = {
  renderQuestionUI: (question, modalEl, badgeEl, textEl) => {
    if (!question) return;

    // Set Info Mata Pelajaran & Kelas (Phase 11)
    const metaInfoEl = document.getElementById("questionMetaInfo");
    if (metaInfoEl) {
      const classObj = DATASET_CATALOG[selectedClass];
      const subjObj = classObj?.subjects.find((s) => s.id === selectedSubject);
      const subjName = subjObj ? subjObj.name : selectedSubject;
      const className = classObj ? classObj.name : selectedClass;
      metaInfoEl.textContent = `📖 ${subjName} (${className})`;
    }

    if (badgeEl) {
      badgeEl.textContent = `${(question.difficulty || "EASY").toUpperCase()}`;
      badgeEl.className = `difficulty-badge ${question.difficulty}`;
    }
    if (textEl) {
    ...
    if (modalEl) modalEl.classList.remove("hide");
  }
};

QuestionEngine.Validator = {
  validateAnswer: (question, userAnswer) => {
    if (!question) return { isCorrect: false, message: "Soal tidak valid" };

    if (question.type === "multiple_choice" || question.type === "true_false") {
      const isCorrect = userAnswer === question.answer;
      return { isCorrect, message: isCorrect ? "Jawaban benar!" : "Jawaban salah!" };
    } else if (question.type === "essay" || question.type === "what_if") {
      const isCorrect = checkEssayAnswer(userAnswer, question.answer, question.input_type);
      return { isCorrect, message: isCorrect ? "Jawaban benar!" : "Jawaban kurang tepat!" };
    }
    return { isCorrect: false, message: "Tipe soal tidak dikenal" };
  }
};

QuestionEngine.ScoreManager = {
  recordResult: (playerIndex, question, isCorrect) => {
    if (playerIndex < 0 || playerIndex >= players.length) return;
    const stats = players[playerIndex].stats;
    if (isCorrect) {
      stats.correct++;
      if (question && question.tileType === "mystery") {
        stats.mysterySolved = (stats.mysterySolved || 0) + 1;
      }
      stats.currentStreak++;
...
QuestionEngine.TimerManager = {
  activeInterval: null,
  start: (duration, timerEl, onTimeout) => {
    ...
    QuestionEngine.TimerManager.activeInterval = setInterval(() => {
      timeLeft--;
      if (timerEl) {
        timerEl.textContent = timeLeft;
  ...
  stop: () => {
    if (QuestionEngine.TimerManager.activeInterval) {
      clearInterval(QuestionEngine.TimerManager.activeInterval);
      QuestionEngine.TimerManager.activeInterval = null;
    }
  }
};
...
let questionPool = {
  ...
  mystery: { multiple_choice: [], true_false: [], essay: [] },
};
...
const loadQuestions = async (classId = selectedClass, subjectId = selectedSubject, difficultyId = globalGameDifficulty) => {
  ...
  const paths = {
    easy: `${basePath}easyQuestion.json`,
    standard: `${basePath}standarQuestion.json`,
    hard: `${basePath}hardQuestion.json`,
    mystery: `${basePath}mysteryQuestion.json`,
  };

  console.log(`[QuestionLoader] Loading questions for Class=${classId}, Subject=${subjectId}, Difficulty=${difficultyId}`);
  ...
  const loadPromises = Object.entries(paths).map(async ([key, path]) => {
    try {
      ...
      // Validasi & normalisasi melalui QuestionEngine
      const validated = QuestionEngine.validateAndNormalizeDataset(data, key);
      if (validated) {
        questionPool[key] = validated;
      } else {
        questionPool[key] = getFallbackQuestions(key);
      }

      console.log(`[QuestionLoader] Loaded ${classId}/${subjectId} (${key}):`, {
        mc: questionPool[key].multiple_choice.length,
        tf: questionPool[key].true_false.length,
        es: questionPool[key].essay.length,
      });
    } catch (error) {
      console.warn(`[QuestionLoader] Primary fetch failed for ${path}, trying root fallback...`, error);
      try {
        const fallbackFilename = key === "standard" ? "standarQuestion.json" : `${key}Question.json`;
        const rootResp = await fetch(`question/${fallbackFilename}`);
        if (rootResp.ok) {
          const rootData = await rootResp.json();
          questionPool[key] = QuestionEngine.validateAndNormalizeDataset(rootData, key) || getFallbackQuestions(key);
        } else {
          questionPool[key] = getFallbackQuestions(key);
        }
      } catch (e) {
  ...
  console.log(`[QuestionLoader] Dataset ${classId} - ${subjectId} ready!`);
};
...
// Helper parser nilai matematika ringan untuk pecahan (1/2, 2/4, 3/6, 0.5, \frac{1}{2})
const parseMathNumericValue = (str) => {
  if (typeof str !== "string") return NaN;

  let s = str.trim()
    .replace(/\\frac\{([^}]+)\}\{([^}]+)\}/g, "$1/$2") // Konversi \frac{a}{b} -> a/b
    .replace(/,/g, ".") // Ganti koma desimal ke titik
    .replace(/[^\d./-]/g, ""); // Ambil hanya digit, titik, minus, dan slash
...
// Helper perbandingan himpunan (Set comparison)
const compareSetAnswer = (userAns, correctAns) => {
  const cleanElems = (str) =>
    str.replace(/[\{}]/g, "")
      .split(/[,;\s]+/)
  ...
  const uElems = cleanElems(userAns);
  const cElems = cleanElems(correctAns);

  if (uElems.length === 0 || uElems.length !== cElems.length) return false;
  return uElems.every((val, index) => val === cElems[index]);
};
...
const checkEssayAnswer = (userAns, correctAns, type) => {
  ...
  const rawCorrect = String(correctAns).trim();

  // Helper pembersih string umum
  const cleanPunctuation = (s) =>
    s.toString()
      .toLowerCase()
      .replace(/[.,\/#!$%\^&\*;:{}=\-_`~()?"'\\{}[\]]/g, "")
      .replace(/\s+/g, " ")
  ...
  let uClean = cleanPunctuation(rawUser);
  let cClean = cleanPunctuation(rawCorrect);

  if (uClean === "") return false;

  // --- TIPE SET (Himpunan) ---
  if (type === "set") {
    if (compareSetAnswer(rawUser, rawCorrect)) return true;
  }
  ...
  if (type === "math" || type === "set") {
    ...
    // 2. Evaluasi Nilai Matematika (Pecahan 1/2 == 0.5 == 2/4 == 3/6 == \frac{1}{2})
    const uNum = parseMathNumericValue(rawUser);
    const cNum = parseMathNumericValue(rawCorrect);

    if (!isNaN(uNum) && !isNaN(cNum)) {
      if (Math.abs(uNum - cNum) < 1e-4) return true;
    }

    // 3. Jika kunci jawaban berisi persamaan (misal "3/6 = 1/2"), cek kesamaan dengan salah satu bagian
    if (rawCorrect.includes("=")) {
      const parts = rawCorrect.split("=");
      for (const part of parts) {
        const partClean = cleanPunctuation(part);
        if (uClean === partClean && partClean !== "") return true;
        const partNum = parseMathNumericValue(part);
        if (!isNaN(uNum) && !isNaN(partNum) && Math.abs(uNum - partNum) < 1e-4) return true;
      }
    }
  ...
  // --- TIPE TEXT (Penjelasan / Kalimat) ---
  if (type === "text") {
    // 1. Cek persis sama setelah abaikan tanda baca dan kapitalisasi
    if (uClean === cClean) return true;

    // 2. Toleran jika jawaban user mengandung kata kunci utama atau sebaliknya
    if (uClean.length >= 3 && cClean.length >= 3) {
      if (cClean.includes(uClean) || uClean.includes(cClean)) {
        if (Math.abs(uClean.length - cClean.length) < 25) return true;
      }
    }
  }
...
const submitAnswer = (question, answer) => {
  ...
  // 1. Validasi Jawaban via QuestionEngine.Validator
  const validationResult = QuestionEngine.Validator.validateAnswer(question, answer);
  const correct = validationResult.isCorrect;
  const message = validationResult.message;

  // 2. Pencatatan Statistik via QuestionEngine.ScoreManager
  const pIdx = currentPlayerTemp - 1;
  QuestionEngine.ScoreManager.recordResult(pIdx, question, correct);
...
const askQuestionOnTile = (playerNo, tileType) => {
  ...
  // 1. Ambil soal via QuestionEngine.Manager
  const q = QuestionEngine.Manager.getRandomQuestionForTile(tileType);
  if (!q) {
    console.error(`Pool soal untuk ${tileType} kosong!`);
    finalizeTurn(playerNo);
    return;
  }

  currentQuestion = q;

  // 2. Render UI via QuestionEngine.Renderer
  const modal = document.querySelector("#questionModal");
  ...
  // 3. Jalankan Timer via QuestionEngine.TimerManager
  const timerEl = document.getElementById("timer");
  QuestionEngine.TimerManager.start(currentQuestion.time_limit || 30, timerEl, handleTimeout);
};
...
const next = async () => {
  ...
  // Memuat dataset soal dinamis berdasarkan pilihan Kelas, Mapel, dan Difficulty (Phase 6)
  await loadQuestions(selectedClass, selectedSubject, globalGameDifficulty);
  ...
  console.log(
    `Game started with ${playersCount} players in ${selectedClass} - ${selectedSubject} (${globalGameDifficulty}). First turn: Player ${currentTurnPlayer}`
  );
};
...
const initialState = () => {
  ...
  console.log(
    `Initial state: playersCount=${playersCount}, currentTurnPlayer=${currentTurnPlayer}, selectedClass=${selectedClass}, selectedSubject=${selectedSubject}`
  );
};
